...
def sai288_hash(data_bytes):
    ...
    for block_start in range(0, len(padded_data), 72):
        ...
        for t in range(64):
            f1 = (S[(t + 1) % 9] ^ M[t % 18]) + int((gamma ** (t / R)) * tau) ^ (((S[(t + 4) % 9] << int(phi * t) % 32) | (S[(t + 4) % 9] >> (32 - int(phi * t) % 32))) & 0xFFFFFFFF)
...
def exponomial_constant(i, gamma, R, tau, S_list, phi):
    ...
    ssum = sum(S_list[j] * (phi ** j) for j in range(min(len(S_list), i+1)))
...
if __name__ == '__main__':
    ...
    block_index = 0
    reward = Decimal('0.0002231668235294118')
